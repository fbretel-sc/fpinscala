...
class ParSpec extends Specification with DataTables {
  ...
  "The following exercises should be correct" >> {
    ...
    "Exercise 7.5.1: parMap." in {
      val as = List(CA("1"), CA("2"), CA("3"), CA("4"), CA("5"), CA("6"), CA("7"), CA("8"))
      val parList = Par.parMap(as) { ca =>
        Thread.sleep(d2)
        ca.a * 2
      }
      ...
      try {
        ...
        if (fut.isDone) {
          ...
          val res = fut.get(safeMaxFor(d2), TimeUnit.MILLISECONDS)
          res must_== List("11", "22", "33", "44", "55", "66", "77", "88")
        }
      } catch {
        case ex: Throwable => ko(ex.getMessage)
      }
    ...
    "Exercise 7.11: choiceN." in {
      ...
      val parC = (es: ExecutorService) => es.submit(CallableCA(d3, "C-"))
      val parD = (es: ExecutorService) => es.submit(CallableCA(d3, "D-"))
    ...
    "Exercise 7.12: choiceMap." in {
      val callableChoice = new Callable[Int] {
        def call(): Int = {
          Thread.sleep(d1)
          1
        }
      ...
      val map = Map(
        (0, (es: ExecutorService) => es.submit(CallableCA(tooLongDuration, "C-"))),
        (1, (es: ExecutorService) => es.submit(CallableCA(d2, "D-")))
      )

      val t0 = System.currentTimeMillis()
      val parChoice = Par.choiceMap(cond)(map)
    ...
    "Exercise 7.13: choiceViaChooser." in {
      ...
      val par1 = (es: ExecutorService) => es.submit(CallableCA(d2, "C-"))
